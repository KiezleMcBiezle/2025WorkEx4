...
class ChatGUI:
    def __init__(self, host='127.0.0.1', port=12345):
        ...
        self.messages = []  # To store messages along with timestamps
        self.ask_username()
    ...
    def setup_main_window(self):
        ...
        # Close button
        close_button = Button(title_bar, text='X', command=self.close_chat, bg='red', fg='white', bd=0, padx=5, pady=2)
        ...
        # Set the chat to check for old messages every 1 minute (60,000 ms)
        self.root.after(60000, self.check_old_messages)
    ...
    def display_msg(self, message):
        ...
        self.messages.append((message, timestamp))  # Store the message with its timestamp
    ...
    def check_old_messages(self):
        ...
        # Remove messages older than 5 minutes (300 seconds)
        self.messages = [
            (msg, timestamp) for msg, timestamp in self.messages
            if current_time - timestamp <= 300]

        # Re-render the messages in the chat window
        self.txt.config(state=NORMAL)
        self.txt.delete(1.0, END)  # Clear all messages
        for msg, timestamp in self.messages:
            self.txt.insert(END, f"\n{msg}")
        self.txt.config(state=DISABLED)

        # Schedule the next check
        self.root.after(60000, self.check_old_messages)  # Check again in 1 minute

    def close_chat(self):
        self.root.quit()  # Close the window gracefully
