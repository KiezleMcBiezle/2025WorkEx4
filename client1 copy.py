...
class Client:
    def __init__(self, HOST, PORT, name, gui_callback):
        ...
        self.gui_callback = gui_callback  # GUI update function
        self.name = name
        ...
        # Sending the username (plain text)
        self.socket.send(self.name.encode('utf-8'))

        # Starting the message reception in the background
            ...
            try:
                ...
                while True:
                    try:
                        ...
                        self.gui_callback(json.dumps(obj))  # transmit back as a JSON string
...
class ChatGUI:
    def __init__(self, host='192.168.55.15', port=12345):
        self.client = None
        self.host = host
        self.port = port
        self.images = []  # Storing images to prevent deletion by the garbage collector
        self.ask_username()
    ...
    def setup_main_window(self):
        ...
        # Text field for chat
        self.txt = Text(self.root, bg="#000000", fg="#ff5533", font='Helvetica 10', width=60)
        ...
        self.entry = Entry(input_frame, bg="#2C3E50", fg="#ff5533", font='Helvetica 10', width=40)
        self.entry.pack(side=LEFT, padx=10)
        self.entry.bind("<Return>", lambda event: self.send_msg())

        send_btn = Button(input_frame, text="Send", font='Helvetica 9 bold', bg='#ff5533', command=self.send_msg)
    ...
    def receive_msg(self, message):
        try:
            ...
            # If not JSON — output as plain text.
            time_str = time.strftime('%H:%M')
    ...
    def display_image(self, sender, img_b64):
        try:
            ...
            # Open the picture in a separate window
    ...
    def open_image_window(self, pil_image):
        ...
        # Scale down if too large (maximum 600x600)
        pil_image.thumbnail((600, 600))

        photo = ImageTk.PhotoImage(pil_image)

        label = Label(win, image=photo)
        label.image = photo  # save it so that it doesn't get deleted
        label.pack()


if __name__ == '__main__':
    ChatGUI('192.168.55.15', 12345)  # Insert the server's IP and port
